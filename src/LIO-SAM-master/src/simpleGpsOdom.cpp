...
public:
    GNSSOdom() : nh("~") {
        ...
        gpsOdomPub = nh.advertise<nav_msgs::Odometry>("/gps_odom", 100, false);     // 经处理后发布的GPS数据
...
private:
    void GNSSCB(const sensor_msgs::NavSatFixConstPtr &msg) {
        ...
        if (!initXyz) {
            ...
            ROS_WARN("Init Orgin GPS LLA  %f, %f, %f", msg->latitude, msg->longitude, msg->altitude);
        ...
        Eigen::Vector3d ecef = gtools.LLA2ECEF(lla);    // 经纬高转换到地心地固坐标系（ECEF）
        Eigen::Vector3d enu = gtools.ECEF2ENU(ecef);    // ECEF到东北天坐标系（ENU）

        ROS_INFO("GPS ENU XYZ : %f, %f, %f", enu(0), enu(1), enu(2));   // ROS_DEBUG、ROS_WARN

        // **********************************************************************************************
        Eigen::Vector3d slam = gtools.ENU2SLAM(enu);  // 东北天（ENU）到slam坐标系
        ...
        if (distance > 0.1) {
            yaw = atan2(enu(1) - prevPos(1),
                        enu(0) - prevPos(0)); // 返回值是此点与原点连线与x轴正方向的夹角
